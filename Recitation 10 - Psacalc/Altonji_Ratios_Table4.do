...
xi: reg trust_relative ln_export_area $baseline_controls, cluster(murdock_name)
predict hat if e(sample), xb
gen beta3=_b[ln_export_area]
* Unrestricted #1
xi: reg trust_relative ln_export_area i.isocode if missing(hat)!=1, cluster(murdock_name)
gen beta1=_b[ln_export_area] if missing(hat)!=1
* Unrestricted #2
xi: reg trust_relative ln_export_area i.isocode age age2 male if missing(hat)!=1, cluster(murdock_name)
gen beta2=_b[ln_export_area] if missing(hat)!=1
* Row 1
gen diff1_3=beta3/(beta1-beta3)
* Row 3
